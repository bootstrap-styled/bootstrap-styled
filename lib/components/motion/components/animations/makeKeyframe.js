 ...
 * @param userKeyframes
 */
function makeKeyframe(makeAnimation, distance) {
  var userKeyframes = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

  var MAX_KEYFRAMES = 50;

  var merge = (0, _immutable.fromJS)(makeAnimation(distance)).mergeDeep(userKeyframes);
  var hashCode = merge.hashCode();
  ...
  var keyframeStr = toKeyframeString(merge.toJS());
