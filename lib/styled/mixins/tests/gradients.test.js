import { gradientX, gradientY, gradientDirectional, gradientXThreeColors, gradientYThreeColors, gradientRadial, gradientStriped } from '../gradients';

describe('bootstrap gradients mixins', function () {
  it('gradientX should return a css', function () {
    var css = gradientX();
  ...
  it('gradientY should return a css', function () {
    var css = gradientY();
    expect(css).not.toContain('undefined');
    expect(css).not.toContain('null');
  ...
  it('gradientDirectional should return a css', function () {
    var css = gradientDirectional();
    expect(css).not.toContain('undefined');
    expect(css).not.toContain('null');
    expect(css).toEqual('\n    background-repeat: repeat-x;\n    background-image: linear-gradient(45deg, #555, #333);\n  '); // eslint-disable-line no-useless-escape
  });
  it('gradientXThreeColors should return a css', function () {
    var css = gradientXThreeColors();
    expect(css).not.toContain('undefined');
    expect(css).not.toContain('null');
  ...
  it('gradientYThreeColors should return a css', function () {
    var css = gradientYThreeColors();
    expect(css).not.toContain('undefined');
    expect(css).not.toContain('null');
  ...
  it('gradientRadial should return a css', function () {
    var css = gradientRadial();
    expect(css).not.toContain('undefined');
    expect(css).not.toContain('null');
    expect(css).toEqual('\n    background-image: radial-gradient(circle, #555, #333);\n    background-repeat: no-repeat;\n  '); // eslint-disable-line no-useless-escape
  });
  it('gradientStriped should return a css', function () {
    var css = gradientStriped();
    expect(css).not.toContain('undefined');
    expect(css).not.toContain('null');
